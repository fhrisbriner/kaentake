...
// MapleNight: the CLOCK-DRIVEN night look is off, and this is the only switch for it.
//
// Our night is not drawn here. It is the server-side background swap in DayNight.java, which
// turns a town's own day layers off and its night layers on through BACKGROUND_EFFECT, so the
// art on screen at night is the map's own night art rather than a colour multiply over its day
// art. Running both would darken every town twice and put this module's moon next to the one
// the map's night bank already draws.
//
// What this switch does NOT touch is the WEATHER darkening: g_fBoost (the active profile's own
// dimming, from weather_profiles.inc) is untouched, so an overcast noon is still dim and a storm
// is still dimmer. Only the day/night curve is held at zero, which also holds the injected sky
// gradient and moon at alpha 0 -- both read g_fNight for their alpha -- so those entries are
// still injected and simply never become visible. Precipitation, cloud, fog, rainbow and
// lightning are all independent of it and run normally.
//
// Set to 1 to hand the night back to this module; then turn OFF the server-side swap in
// DayNight.java, or the two fight over the same town.
static constexpr int kWeatherNightVisuals = 0;

// MapleNight: does this module get to touch the map's OWN background layers? It does not, and
// this is the only switch for that.
//
// The backgrounds are the day/night system. Character.applyVisibility walks the map's bgVisMask
// (built from each back entry's maoFlags) and sends SET_BACK_EFFECT per layer; the client's
// CMapLoadable::OnSetBackEffect @0x006445C5 animates that layer's alpha vector between 0 and 255
// over the transition, fading a town's day art out and its night art in. Nothing else may write
// those layers, and this module was writing them three separate ways -- a per-frame colour, a z
// rewrite that promotes them in front of the injected band, and the FLAG_BARESKY `visible`
// toggle. With the switch off, the map's backs are simply never captured, which removes all
// three at once.
//
// What this costs, and it is worth knowing before flipping it back: the z promotion existed to
// pull a map's own hills and trees in FRONT of the injected weather band. Without it, cloud and
// fog sheets sit in front of the map's distant scenery rather than behind it. Rain, snow, the
// ground effects, the tile/object/NPC tint and everything else are unaffected -- none of them
// go through the back list.
//
// Set to 1 only if the server-side swap is being retired, and turn off Character's day/night
// with it. The two cannot share the same layers.
static constexpr int kWeatherOwnsMapBacks = 0;
...
static void EffectiveTint(float* pr, float* pg, float* pb) {
    ...
    // The golden dawn/dusk warm shift is part of the clock's night look, so it goes with it.
    // Without this a daytime storm would still be warm-tinted at the hours the clock calls
    // dusk, which is a sunset colour with no sunset on screen to explain it.
    if (!kWeatherNightVisuals) {
        *pr = r; *pg = g; *pb = b;
        return;
    }
...
static float NightFromMinute(float fMinute) {
    // Held at full day: our night is the server-side background swap, not this curve. See
    // kWeatherNightVisuals. The weather profile's own boost is applied separately in
    // EffectiveNight() and is deliberately not affected.
    if (!kWeatherNightVisuals) {
        return 0.0f;
    }
...
void Weather_HandleWorldState(CInPacket* pPacket) {
    ...
    // DIAGNOSTIC (temporary): proves the packet reached the handler and decoded sanely.
    // Keyed on the sky so a steady sky logs once and a change logs again.
    LOG_ONCE_PER_ID(uSky, "weather: 0x373D sky=%u minute=%d msPer=%d flags=0x%02X elapsed=%d "
                          "palette=%d rainbow=%d", uSky, nMinute, nMsPer, uFlags, nElapsed,
                    nPalette, nRainbow);
...
// Fields whose ropes and ladders are FIXED, and so must not be given the rope bend.
//
// El Nath, 211xxxxxx. Its climbables are ice ladders and taut lines pinned to the rock face
// rather than the slack hanging rope the bend's amplitude was calibrated against, so swaying
// them looks like the mountain moving rather than the rope.
//
// Orbis, 200xxxxxx, for the same reason from the opposite direction: the tower's climbables are
...
// shared with regions where the sway IS right.
...
// Writes the whole ARGB, opacity included, which is safe only because of what is in these
// lists. Tiles, objects and NPCs carry no animated alpha, and the map's own BACKS -- the one
// thing here that does -- are no longer captured at all. See kWeatherOwnsMapBacks.
...
IWzGr2DLayerPtr* __fastcall IWzGr2D__CreateLayer_hook(
    ...
    if (pResult && *pResult) {
        try {
            ...
            } else if (g_bCaptureBack && !kWeatherOwnsMapBacks) {
                // MapleNight: the map's own background layers belong to the SERVER-SIDE
                // day/night swap, and this module must not touch them. Dropping the capture
                // here is what enforces that, because it is the single point all three
                // interferences hang off: the layer never enters g_vBacks (so ApplyList never
                // writes its colour), never enters g_vSkyBackLayer (so FLAG_BARESKY cannot
                // toggle its `visible`), and never reaches the HILL_Z promotion below (so its
                // z is left exactly where the engine put it).
                //
                // Why it had to go. CMapLoadable::OnSetBackEffect @0x006445C5 is what
                // Character.applyVisibility drives, once per entry in the map's bgVisMask,
                // and it animates each layer's alpha VECTOR between 0 and 255 over the
                // transition. That is the whole day/night system: a map's day and night art
                // are two back entries faded against each other. Any other writer on those
                // same layers is racing it, and this module was writing them every frame.
    ...
    try {
        ...
        if (ContainsI(sL0, L"rope") || ContainsI(sL0, L"ladder")
         ...
         || ContainsI(sL2, L"rope") || ContainsI(sL2, L"ladder")) {
            // El Nath's are RIGID: fixed ice ladders and taut climbing lines bolted to the
            // cliff, not the loose hanging rope the bend was calibrated on. Swaying them reads
            // as the mountain wobbling. Left as SWAY_NONE rather than given a smaller amplitude
            // because there is no amount of sway that looks right on a ladder frozen into rock.
            //
            // Only the ropes. El Nath's foliage, such as it is, still bends, and the region is
            // permanently snowbound so there is usually a wind to justify it.
            if (!IsNoRopeSwayField(CurrentFieldID())) {
...
void CMapLoadable::LoadMap_hook() {
    ...
    try {
        ...
        // DIAGNOSTIC (temporary): one line per field entered, so a map that renders nothing
        // can be told apart from a map the sky table never recognised.
        LOG_ONCE_PER_ID(CurrentFieldID(), "weather: field %d bSky=%d",
                        CurrentFieldID(), (int)bSky);
        g_bFxBand = bSky;
    ...
    // THE SKY IS REGION STATE AND cDOES NOT TRAVEL. The clock does: it is a world fact, so
...
void AttachWeatherMod() {
    LogInfo("weather: AttachWeatherMod installing"); // DIAGNOSTIC (temporary)
