...
// MapleNight: street lamps are OFF, and this is the only switch for them.
//
// The file still ships and still compiles because weather.cpp calls into it unguarded --
// Lamps_Inject, Lamps_Update, Lamps_RelightScenery and Lamps_OnLeaveField are all reached from
// the field pipeline with no #if around them, so deleting the translation unit is a link error
// rather than an absent feature. The two places that would put a lamp on screen are gated on
// this instead: Lamps_Inject returns before it plans anything, and Lamps_Attach does not install
// the CMapLoadable::MakeObj detour at all, so no map object is even inspected.
//
// It is also the right switch for our setup on its own terms: the photocell reads
// Weather_EffectiveNight(), which the night-visuals switch in weather.cpp holds at the weather
// boost alone, so with lamps on they would light for a daytime storm and stay dark through our
// actual night.
//
// Set to 1 to bring them back; that needs Map/Obj/MapleMoonLamp.img and MapleMoonLamp2.img in
// Data/, and kWeatherNightVisuals in weather.cpp turned on to drive the photocell.
static constexpr int kLampsEnabled = 0;
...
void Lamps_Inject(CMapLoadable* pField, bool bSky) {
    ...
    //
    // kLampsEnabled is checked here rather than at every call site: this is the one function
    // that decides a map has lamps, so returning early leaves the glow plan and the lamp-field
    // flag cleared above, and everything downstream (Update, RelightScenery, the preview) walks
    // empty state.
    if (!kLampsEnabled || !pField || !bSky) {
        return;
    }
...
void Lamps_Attach() {
    // Installed even when the lamp layer is off, and it has to be. The claim this used to
    // carry -- that with kLampsEnabled = 0 the detour "can only ever decline to act" -- was
    // wrong: MakeObj_hook's first statement is Weather_NoteObjProp, which is the ONLY place
    // any object is ever classified. It is what sets the sway kind, the plant flag, the
    // mirror flag, the entry's own coordinates and the path hash, for every object on the
    // map. Skip the detour and every object stays SWAY_NONE, so foliage sway silently plans
    // nothing on every map in the game -- 516 objects on Henesys Hunting Ground I, 255 of
    // them nature-category foliage and 156 rope or ladder, and not one of them swaying.
    //
    // The lamp half of the hook stays correctly gated without any test here: with the layer
    // off Lamps_Inject returns before it sets g_bLampField, so the block that reads the
    // kaentakeLamp / kaentakeGlow tags never runs and neither capture flag is ever armed.
